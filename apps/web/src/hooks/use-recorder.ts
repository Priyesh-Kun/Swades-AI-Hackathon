...
import type { ChunkUploadStatus } from "@/lib/upload";
...
async function createSession(sessionId: string): Promise<void> {
  await fetch(`${SERVER_URL}/api/chunks/sessions/create`, {
    method: "POST",
...
async function stopSession(sessionId: string): Promise<void> {
  await fetch(`${SERVER_URL}/api/chunks/sessions/${sessionId}/stop`, {
    method: "POST",
  });
}
...
export function useRecorder(options: UseRecorderOptions = {}) {
  ...
  const createAndUploadChunk = useCallback(
    (merged: Float32Array) => {
      const currentSessionId = sessionIdRef.current;
      const currentIndex = chunkIndexRef.current;
      const chunkId = `${currentSessionId}-${currentIndex}`;
      ...
      // Upload in background (OPFS persist → server → delete from OPFS)
      uploadChunk({
        chunkId,
        ...
        setChunks((prev) =>
          prev.map((c) =>
            c.id === chunkId
              ? {
                ...c,
                uploadStatus: result.ok ? "acked" : "failed",
                error: result.error,
              }
              : c,
          ),
  ...
  }, [deviceId, chunkThreshold, createAndUploadChunk]);

  const stop = useCallback(() => {
    flushChunk();

    // Signal server that session is done
    const sid = sessionIdRef.current;
    if (sid) {
      stopSession(sid).catch(console.error);
    }
